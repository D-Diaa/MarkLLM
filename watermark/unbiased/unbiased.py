...
class UnbiasedWatermark(BaseWatermark):
    ...
    def generate_watermarked_texts(self, prompts: list[str], *args, **kwargs) -> list[str]:
        """Generate watermarked text."""
        ...
        # Encode prompt
        encoded_prompts = self.config.generation_tokenizer(prompts, return_tensors="pt", add_special_tokens=True, padding=True).to(
            self.config.device)
        # Generate watermarked text
